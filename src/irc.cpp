...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MBITTEST2\r");
            Send(hSocket, "WHO #MBITTEST2\r");
        } else {
            // randomly join #MBIT00-#MBIT05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MBIT%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MBIT%02d\r", channel_number).c_str());
        }
